...
- **支出凭证**：仅已通过申请可回填，同一拨付单累计支出不得超过拨付金额；凭证回填后为「待核验」，**经平台核验通过后**才进入捐赠人可见口径。
- **核验口径**：捐赠人公示的支出凭证仅含已核验记录；汇总中的「已用金额」= 已核验凭证金额合计，待核验凭证不计入；待核验凭证仅供所属组织与平台后台查看。
- **核验幂等与鉴权**：仅平台管理员可核验；重复核验、核验后再次核验、并发核验均返回 409 冲突（行锁 + 状态守卫，恰好成功一次）；非管理员核验返回 403。
- **结项冻结**：存在待审核申请时禁止结项；结项后禁止再申请、再审核拨付。
- **并发安全**：申请/审核/核验在数据库事务内对项目、申请、凭证行加排他锁（MySQL `FOR UPDATE`），叠加按资源维度的进程内互斥与唯一索引，杜绝并发超额、重复拨付与重复核验。
- **数据模型**：`DisbursementApplication`（用款申请）、`DisbursementOrder`（拨付单）、`ExpenseVoucher`（支出凭证，含核验人/核验时间），申请人、审核人、审核/核验意见与时间全程留痕。
...
| 项目结项后再申请或再拨 | 409 已结项 |
| 凭证累计超过拨付金额 | 409 超额 |
| 重复 / 并发核验凭证、核验后再次核验 | 409 已核验冲突（仅一次成功） |
| 非平台管理员核验凭证 | 403 |
| 非本项目所属组织操作 | 403 |
...
- `internal/service/service_disbursement_test.go`：申请→审核→唯一拨付单→回填→平台核验→汇总→公示主流程；超额申请、重复审核、结项后再拨、凭证超额、跨组织越权、未过审项目申请、驳回释放额度，以及并发场景（并发申请不突破已筹、并发审核仅一张拨付单、**凭证重复/越权/并发核验**、待核验凭证不对捐赠人公示且不计入已用）。
